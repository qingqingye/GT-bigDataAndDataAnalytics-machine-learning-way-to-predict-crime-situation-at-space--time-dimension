...
def get_feature_result():
    ...
    lines = 0
    df_time = pd.read_csv('timegroup_from_stkde_small_one_year.csv', header=None, names=col_names)
    df_space = pd.read_csv('spacegroup_from_stkde_small_one_year.csv', header=None, names=[i for i in range(6000)])

    df_time.sort_values(0, inplace=True)
    for index, row in df_time.iterrows():
        ...
        for gird_ind in range(1, 901):
            ...
            avg_near_day, avg_week_day = sum(near_day)/len(near_day), sum(same_weekday)/len(same_weekday)
            near10, near20, near30 = top_60_neighbor_features(row, gird_ind, row[x_ind], row[y_ind])
            allfeatures.append([near10, near20, near30, avg_near_day, avg_week_day])
