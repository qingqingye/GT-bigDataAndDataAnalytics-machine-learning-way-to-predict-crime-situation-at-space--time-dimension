...
df = pd.read_csv("files/refined_data.csv", usecols=[3, 4, 5], nrows=10000)
inX = np.array(df["x_dist"])
inY = np.array(df["y_dist"])
inX = np.round(inX/100, 2).tolist()
inY = np.round(inY/100, 2).tolist()
temp_inT = df["modified_time"].tolist()
...
#
# pFile.close()
xmin, xmax, ymin, ymax, zmin, zmax = min(inX), max(inX), min(inY), max(inY), min(temp_inT), max(temp_inT)
